...
import com.lckiss.doubledatepicker.util.DateUtils;
import com.lckiss.doubledatepicker.widget.WheelView;
...
import static com.lckiss.doubledatepicker.popup.BottomPopup.WRAP_CONTENT;
...
public class DatePicker extends WheelPicker {
    ...
    private int selectedYearIndex2 = 0, selectedMonthIndex2 = 0, selectedDayIndex2 = 0;
    ...
    public void setSelectedItem(int year, int month, int day) {
        ...
        selectedYearIndex2 = selectedYearIndex;
        selectedMonthIndex2 =selectedMonthIndex;
        selectedDayIndex2 =selectedDayIndex;
    }
    ...
    protected View makeCenterView() {
        ...
        beginText.setPadding(50,20,0,0);
        ...
        yearViewBottom.setLineVisible(lineVisible);
        yearViewBottom.setLineColor(lineColor);
        ...
        monthViewBottom.setLineVisible(lineVisible);
        monthViewBottom.setLineColor(lineColor);
        ...
        dayViewBottom.setLineVisible(lineVisible);
        dayViewBottom.setLineColor(lineColor);
        ...
        if (mode == YEAR_MONTH) {
            dayViewBottom.setVisibility(View.GONE);
            dayTextView.setVisibility(View.GONE);
        } else if (mode == MONTH_DAY) {
            yearViewBottom.setVisibility(View.GONE);
            yearTextView.setVisibility(View.GONE);
        }
        if (mode != MONTH_DAY) {
            if (!TextUtils.isEmpty(yearLabel)) {
            ...
            if (selectedYearIndex2 == 0) {
                yearViewBottom.setItems(years);
            } else {
                yearViewBottom.setItems(years, selectedYearIndex2);
            }
            yearViewBottom.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
                @Override
                public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                    selectedYearIndex2 = selectedIndex;
                    //需要根据年份及月份动态计算天数
                    days.clear();
                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(item), stringToYearMonthDay(months.get(selectedMonthIndex)));
                    for (int i = 1; i <= maxDays; i++) {
                    ...
                    if (selectedDayIndex2 >= maxDays) {
                        //年或月变动时，保持之前选择的日不动：如果之前选择的日是之前年月的最大日，则日自动为该年月的最大日
                        selectedDayIndex2 = days.size() - 1;
                    }
                    dayViewBottom.setItems(days, selectedDayIndex2);
                }
            });
        }
        if (!TextUtils.isEmpty(monthLabel)) {
        ...
        if (selectedMonthIndex2 == 0) {
            ...
            monthViewBottom.setItems(months, selectedMonthIndex2);
        }
        monthViewBottom.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            @Override
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                selectedMonthIndex2 = selectedIndex;
                if (mode != YEAR_MONTH) {
                    //年月日或年月模式下，需要根据年份及月份动态计算天数
                    days.clear();
                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(years.get(selectedYearIndex)), stringToYearMonthDay(item));
        ...
        if (mode != YEAR_MONTH) {
            ...
            if (selectedDayIndex2 == 0) {
                dayViewBottom.setItems(days);
            } else {
                dayViewBottom.setItems(days, selectedDayIndex2);
            }
            dayViewBottom.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
                @Override
                public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                    selectedDayIndex2 = selectedIndex;
                }
            });
        }
    ...
    protected void onSubmit() {
        if (onDatePickListener != null) {
            ...
            String year2 = getSelectedYear2();
            String month2 = getSelectedMonth2();
            String day2 = getSelectedDay2();

            ((OnYearMonthDayPickListener) onDatePickListener).onDatePicked(year, month, day,year2, month2, day2);
        }
    }
     ...
     * @return the selected year
     */
    public String getSelectedYear2() {
        return years.get(selectedYearIndex2);
    }
    ...
    public String getSelectedMonth2() {
        return months.get(selectedMonthIndex2);
    }
     ...
     * @return the selected day
     */
    public String getSelectedDay2() {
        return days.get(selectedDayIndex2);
    }
    ...
    public interface OnYearMonthDayPickListener extends OnDatePickListener {
         ...
         * @param day   the day
         */
        void onDatePicked(String year, String month, String day, String year2, String month2, String day2);
